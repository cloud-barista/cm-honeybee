...
// Parse turns a v12 `nvidia-smi -q -x` document into the common GPU model.
	...
	for i, g := range log.GPU {
		...
		nv := infra.NVIDIA{
			...
			Performance: infra.NVIDIAPerformance{
				...
				PowerDraw: common.FirstFloat64(g.GpuPowerReadings.PowerDraw,
					g.GpuPowerReadings.InstantPowerDraw),
