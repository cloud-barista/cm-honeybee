...
// Parse turns a v13 `nvidia-smi -q -x` document into the common GPU model.
func Parse(data []byte) ([]infra.NVIDIA, error) {
	...
	driverVersion := common.FirstStr(log.DriverVersion, log.KmdVersion)
	cudaVersion := common.FirstStr(log.CudaVersion, log.CudaUmdVersion)
	...
	for i, g := range log.GPU {
		...
		nv := infra.NVIDIA{
			...
			Performance: infra.NVIDIAPerformance{
				...
				PowerDraw: common.FirstFloat64(g.GpuPowerReadings.InstantPowerDraw,
					g.GpuPowerReadings.AveragePowerDraw, g.GpuPowerReadings.PowerDraw),
				PowerLimit: common.FirstFloat64(g.GpuPowerReadings.CurrentPowerLimit,
					g.GpuPowerReadings.PowerLimit),
				...
				ClocksEventReasons: common.ClocksEventReasons(map[string]string{
					"gpu_idle":                    g.ClocksEventReasons.GpuIdle,
					"applications_clocks_setting": g.ClocksEventReasons.ApplicationsClocksSetting,
					"sw_power_cap":                g.ClocksEventReasons.SwPowerCap,
					"hw_slowdown":                 g.ClocksEventReasons.HwSlowdown,
					"hw_thermal_slowdown":         g.ClocksEventReasons.HwThermalSlowdown,
					"hw_power_brake_slowdown":     g.ClocksEventReasons.HwPowerBrakeSlowdown,
					"sync_boost":                  g.ClocksEventReasons.SyncBoost,
					"sw_thermal_slowdown":         g.ClocksEventReasons.SwThermalSlowdown,
					"display_clocks_setting":      g.ClocksEventReasons.DisplayClocksSetting,
				}),
			},
...
// eccErrors folds the v13 uncorrectable SRAM split back into a single counter,
// since parity and SECDED errors are both uncorrectable SRAM errors.
func eccErrors(c eccCounts) infra.NVIDIAECCErrors {
	uncorrectable := common.Uint64(c.SramUncorrectable)

	parity := common.Uint64(c.SramUncorrectableParity)
	secded := common.Uint64(c.SramUncorrectableSecded)
	if parity != nil || secded != nil {
		var sum uint64
		if parity != nil {
			sum += *parity
		}
	...
	return infra.NVIDIAECCErrors{
		...
		SRAMUncorrectable: uncorrectable,
